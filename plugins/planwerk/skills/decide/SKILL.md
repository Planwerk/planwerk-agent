...
repo, say so and stop. Then check the checkout per `github.md`, The checkout,
because a decision confirmed against a stale checkout is not confirmed at all.
...
naming your model id, the same rule Phase 4 applies to the Meta Issue. Then read
its comments (`gh issue view <number> --repo <owner/repo> --comments`) and note
whether one is an implementation plan comment: headed
`## Implementation Plan (issue #N)`, with a footer beginning
`_Implementation plan generated by [planwerk-agent]`. `planwerk-agent implement`
reuses the most recent one verbatim, and it was written against the body as it
stood before your correction. Phase 7 names every such sibling.
...
- Every corrected sibling that carries an implementation plan comment. Its
  implementation starts with
  `planwerk-agent implement <issue-ref> --no-plan-reuse`: a bare run reuses the
  plan written against the body before your correction.
...
`/planwerk:elaborate` or `planwerk-agent implement` — with `--no-plan-reuse`
when you corrected that Sub Issue and it carries a plan comment — or, if a
Broken item still stands unresolved, say that it blocks everything downstream
until it is fixed.
