...
# Implement an issue that has not been elaborated, without being asked first
...
`--implement-worker-model claude-opus-5-5`) when you want the footer to name
...
2. **Fetch Issue**: Title, body, URL, and state are fetched via `gh issue view`. A body that continues in comments (an elaborated plan over its body's limit, see [Elaborate an issue](/how-to/elaborate-an-issue#large-plans-stay-whole-a-budget-and-a-continuation-comment)) is merged with its continuation comments first; a part the body announces and no comment carries aborts the run rather than planning against a truncated issue. A body with no Acceptance Criteria heading has not been elaborated: the run asks whether to implement it anyway (`y/N`) before anything is cloned, and a no aborts with the `elaborate` invocation to run first. A non-TTY run refuses instead; `--allow-unelaborated` skips the question.
...
8. **Review-and-Fix Pass (default-on)**: After the simplify pass — a full run is **implement → simplify → review → finalize** — this pass runs the same finders and the same finding hygiene the [`review`](/how-to/review-a-pr) command runs (package `internal/hygiene`), then folds each surviving fix into the commit it belongs to (`git commit --fixup` + `git rebase --autosquash`) on the local branch — no push, since no PR exists yet. Unlike simplify, this pass is allowed and expected to add regression tests.
   ...
   - It runs as a **bounded loop**: after each apply it re-reviews — the first round over the whole branch diff, every round after it over just what the previous round's fixes changed — and, while the finder still reports findings that survive hygiene, fixes them again — stopping when the finder comes back clean, when a round yields no survivors, when an apply escalates (`STATUS: BLOCKED` / `NEEDS_CONTEXT`), when an apply resolves none of the findings it was handed (the branch is unchanged, so a further round could only re-report them), or after `--max-review-iterations` rounds (default 3), in which case it notes the findings still unresolved when the budget ran out. Each round's report is posted as a comment on the source issue (best-effort — a failed post never aborts the run). When the review finds nothing on the first round it is a clean no-op: no commit, no issue comment beyond a short "review found nothing" note on stdout. The whole pass is non-fatal — a failed or escalated review never changes the run's exit code. The read-only `--verify` flag remains available for a report-only run. Disable the whole pass with `--no-review`, or just the specialist fan-out with `--no-specialists`.
...
10. **Verification (optional)**: With `--verify`, an independent pass runs over the actual committed diff, not the implementer's self-report: a session diffs the feature branch against the issue's Acceptance Criteria, and any unmet criteria it finds are then fed into the same review applier the review-and-fix pass uses, so the gaps are fixed on the local branch before the finalize step opens the PR (that apply is non-fatal too, and a clean pass — or a run with no applier wired — stays render-only). The pass is non-fatal — a finding is reported, it does not fail the run.
11. **Finalize Session**: With the simplify and review passes done, a fresh auto-mode session opens the draft pull request last, so it lands already simplified and self-reviewed. It resolves the base branch from `origin/HEAD`, pushes the feature branch, and runs `gh pr create --draft` with a description that walks the reviewer through the commits in order and links the issue with the closing keyword `Closes #N`. This is the run's deliverable, so — unlike the passes above — a genuine failure to push or open the PR is **fatal** (the branch is committed locally, and the operator is told the PR was not created). A branch that carries no commits over the base opens no PR and is not an error. A failure keeps the branch resumable (pushed to `origin` in clone mode), so the next run picks it up.

When a run stops after its implement session finished — its `DONE` implementation report is on the issue and a later pass hit a usage limit — the next run for the same issue resumes the branch and continues from the pass that stopped: it skips the implement session, skips the simplify pass when its report was posted, and re-enters the review loop at the round after the last posted review report, scoped to that round's fixes. See the [resume notes in the CLI reference](/reference/cli#implement).
