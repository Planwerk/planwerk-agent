...
2. **Fetch Issue**: Title, body, URL, and state are fetched via `gh issue view`. A body that continues in comments (an elaborated plan over GitHub's 65,536-character cap, see [Elaborate an issue](/how-to/elaborate-an-issue#large-plans-stay-whole-a-budget-and-a-continuation-comment)) is merged with its continuation comments first; a part the body announces and no comment carries aborts the run rather than planning against a truncated issue.
