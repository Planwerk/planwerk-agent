...
1. READ the issue body in full. Extract Acceptance Criteria, Non-Goals, Affected Areas, and References into your working notes. Extract the user stories from the issue body's **User Stories** section when the elaborated issue supplies them; otherwise derive them yourself, subject to the same proportionality rule — generate exactly as many stories as the issue REQUIRES, emit none for purely mechanical work (refactors, CI fixes, dependency bumps, rebases), and never invent a synthetic persona.
...
## Implementation Plan (final output)
   ...
   ### Change Set
   - <path> — <create | modify | delete>: <what changes and why, naming the exact symbols involved; reference the user story it serves when stories are present>
   ### Commit Sequence
