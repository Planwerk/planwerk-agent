...
// ("claude-opus-5-5") is known only at runtime, so each Claude session returns
...
// id: "with Claude:claude-opus-5-5" when model is non-empty, and a bare
...
// CommentFooter renders the attribution line the commands append to the
// comments they post on GitHub: "_<lead> <tool> <command> <assistant>_", for
// example "_Fix report generated by [planwerk-agent](url) e1efd0d fix with
// Claude:claude-opus-5-5_". lead names the artifact and the verb, command the
// subcommand that produced it, model the resolved Claude model.
func CommentFooter(lead, command, model string) string {
	return "_" + lead + " " + Tool() + " " + command + " " + AssistantWith(model) + "_"
}
