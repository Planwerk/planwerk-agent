...
func fixCommentFooter(model string) string {
	return attribution.CommentFooter("Fix report generated by", "fix", model)
}
...
func (r *Runner) postFixComment(w io.Writer, opts Options, owner, repo string, number int, fixReport, model string) {
	...
	github.PostBestEffort(w, "fix report", fmt.Sprintf("PR #%d", number), func() (string, error) {
		return r.GitHub.AddPRComment(owner, repo, number, formatFixComment(fixReport, model))
	})
}
