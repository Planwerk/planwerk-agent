...
func (p Pass) postComment(w io.Writer, command, reportBody, model string) {
	github.PostBestEffort(w, "capture proposals", "", func() (string, error) {
		return p.PostComment(formatComment(reportBody, command, model))
	})
}
...
func commentFooter(command, model string) string {
	return attribution.CommentFooter("Capture proposals generated by", command, model)
}
